...
export async function answerQuestion(
  ...
  logger.info("Processing query for context-window", {
  ...
  const windowChars = cfg.contextWindowSize ?? 1200;
  const expanded = stitchRetrievedChunks(retrieved, windowChars);
  const prompt = buildPrompt(trimmedQuestion, expanded);
  ...
  return { answer, retrieved: expanded, prompt };
}
...
async function interactiveQuery(): Promise<void> {
  ...
  const configPath =
    ...
    path.resolve(__dirname, "../config/context-window.config.json");
  ...
  logger.info("Context-window query CLI ready. Type 'exit' to quit.");
  ...
  while (true) {
    ...
    console.log("\nAnswer:\n", answer, "\n");
    logger.info("Query summary (context-window)", {
      question,
...
if (require.main === module) {
  interactiveQuery().catch((err) => {
    logger.error("Context-window query CLI failed", { err });
    process.exitCode = 1;
  });
}
