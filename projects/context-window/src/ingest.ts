...
export async function runIngestion(
  ...
  logger.info("Reading documents for context-window", { dataPath: cfg.dataPath });
  const documents = readDocs(cfg.dataPath);

  logger.info("Chunking documents for context-window", {
...
async function main(): Promise<void> {
  ...
  logger.info("Loading config for context-window", { configPath });
  const cfg = loadRagConfig(configPath);

  const result = await runIngestion(cfg);
  logger.info("Context-window ingestion complete", {
...
if (require.main === module) {
  main().catch((err) => {
    logger.error("Context-window ingestion failed", { err });
    process.exitCode = 1;
  });
}
