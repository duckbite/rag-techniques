...
import { stitchRetrievedChunks } from "../../../shared/typescript/utils/evaluation";
...
export async function answerQuestion(
  ...
  logger.info("Processing query for relevant-segments", {
  ...
  const maxChars = cfg.segmentMaxChars ?? 800;
  const stitched = stitchRetrievedChunks(retrieved, maxChars);
  const prompt = buildPrompt(trimmedQuestion, stitched);
  ...
  return { answer, retrieved: stitched, prompt };
}
...
async function interactiveQuery(): Promise<void> {
  ...
  const configPath =
    ...
    path.resolve(__dirname, "../config/relevant-segments.config.json");
  ...
  logger.info("Relevant-segments query CLI ready. Type 'exit' to quit.");
  ...
  while (true) {
    ...
    console.log("\nAnswer:\n", answer, "\n");
    logger.info("Query summary (relevant-segments)", {
      question,
...
if (require.main === module) {
  interactiveQuery().catch((err) => {
    logger.error("Relevant-segments query CLI failed", { err });
    process.exitCode = 1;
  });
}
