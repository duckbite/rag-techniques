...
const cfg: RagConfig = {
  ...
  segmentMaxChars: 1000
};

describe("relevant-segments query", () => {
  it("stitches adjacent chunks into longer segments", async () => {
    ...
    expect(result.retrieved[0].content).toContain("Part A");
    expect(result.retrieved[0].content).toContain("Part B");
    expect(chatClient.lastMessages?.[0].content).toContain("Part A");
    expect(chatClient.lastMessages?.[0].content).toContain("Part B");
  });
});
