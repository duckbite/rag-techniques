...
export async function runIngestion(
  ...
  logger.info("Reading documents for relevant-segments", { dataPath: cfg.dataPath });
  const documents = readDocs(cfg.dataPath);

  logger.info("Chunking documents (baseline) for relevant-segments", {
...
async function main(): Promise<void> {
  ...
  logger.info("Loading config for relevant-segments", { configPath });
  const cfg = loadRagConfig(configPath);

  const result = await runIngestion(cfg);
  logger.info("Relevant-segments ingestion complete", {
...
if (require.main === module) {
  main().catch((err) => {
    logger.error("Relevant-segments ingestion failed", { err });
    process.exitCode = 1;
  });
}
