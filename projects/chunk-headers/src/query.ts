...
export async function answerQuestion(
  ...
  logger.info("Processing query for chunk-headers", { question: trimmedQuestion, topK: cfg.topK });
...
async function interactiveQuery(): Promise<void> {
  ...
  logger.info("Chunk-headers query CLI ready. Type 'exit' to quit.");
  ...
  while (true) {
    ...
    console.log("\nAnswer:\n", answer, "\n");
    logger.info("Query summary (chunk-headers)", {
      question,
...
if (require.main === module) {
  interactiveQuery().catch((err) => {
    logger.error("Chunk-headers query CLI failed", { err });
    process.exitCode = 1;
  });
}
