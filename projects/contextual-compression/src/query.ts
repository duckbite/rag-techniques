...
import { compressRetrievedContext } from "../../../shared/typescript/utils/llm";
...
export async function answerQuestion(
  ...
  logger.info("Processing query for contextual-compression", {
  ...
  const prompt = [
    "You are a helpful assistant answering questions based only on the provided compressed context.",
    "If the answer is not in the context, say you don't know.",
    "",
    "Compressed context:",
    compressedContext || "(no relevant context)",
    "",
    `Question: ${trimmedQuestion}`,
    "Answer:"
  ].join("\n");
...
async function interactiveQuery(): Promise<void> {
  ...
  logger.info("Contextual-compression query CLI ready. Type 'exit' to quit.");
  ...
  while (true) {
    ...
    console.log("\nAnswer:\n", answer, "\n");
    logger.info("Query summary (contextual-compression)", {
      question,
...
if (require.main === module) {
  interactiveQuery().catch((err) => {
    logger.error("Contextual-compression query CLI failed", { err });
    process.exitCode = 1;
  });
}
