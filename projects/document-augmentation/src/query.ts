...
export function formatRetrievedChunks(chunks: RetrievedChunk[]): string {
  ...
  return chunks
    .map((chunk, idx) => {
      ...
      const isAugmented = chunk.metadata?.augmentation === "qa";
      const label = isAugmented ? "Q/A pair" : "Source chunk";
      return [
        `Chunk ${idx + 1} (${label}, score=${chunk.score.toFixed(3)})`,
        `Source: ${source}`,
        chunk.content
      ].join("\n");
    })
...
export async function answerQuestion(
  ...
  logger.info("Processing query for document-augmentation", {
...
async function interactiveQuery(): Promise<void> {
  ...
  logger.info("Document-augmentation query CLI ready. Type 'exit' to quit.");
  ...
  while (true) {
    ...
    console.log("\nAnswer:\n", answer, "\n");
    logger.info("Query summary (document-augmentation)", {
      question,
...
if (require.main === module) {
  interactiveQuery().catch((err) => {
    logger.error("Document-augmentation query CLI failed", { err });
    process.exitCode = 1;
  });
}
